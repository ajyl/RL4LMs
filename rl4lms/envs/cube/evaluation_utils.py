...
from rl4lms.envs.cube.metric import BaseMetric
...
def evaluate_on_samples(
    ...
    for batch in tqdm(list(get_batch(samples, batch_size)), desc="Evaluating"):
        batch_generated_texts = generate_text(
            policy, tokenizer, batch, dt_control_token, gen_kwargs
        )
...
def generate_text(
    ...
    prompt_texts = [
        dt_control_token + sample.cube_state_str for sample in samples
    ]
    generated_texts = policy.generate(
        tokenizer, prompt_texts, gen_kwargs=gen_kwargs
    ).gen_texts
