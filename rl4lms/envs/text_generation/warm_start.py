...
class OnPolicyWarmStartMixin:
    ...
    def load_from_dict(self, state_dict: Dict[str, Any]) -> Dict[str, Any]:
        if state_dict is not None:
            self._kl_controller.load_from_state_dict(state_dict["kl_controller_state"])
...
class OffPolicyWarmStartMixin:
    ...
    def load_from_dict(self, state_dict: Dict[str, Any]) -> Dict[str, Any]:
        if state_dict is not None:
            self.replay_buffer.load_from_state_dict(state_dict["replay_buffer"])
...
class TrainerWarmStartMixin:
    def _get_recent_ckpt_path(self, tracker: Tracker):
        ...
        sorted_ckpts = sorted(
            checkpoints, reverse=True, key=lambda ckpt: int(ckpt.split("_")[1])
        )
    ...
    def load_trainer_state(self, tracker: Tracker):
        ...
        try:
            ...
            tracker.log_info(f"Checkpoint may be corrupted...skipping warm start")
            self._policy_state_dict = None
            self._alg_state_dict = None
            self._trainer_state = {
                "current_iter": 0,
